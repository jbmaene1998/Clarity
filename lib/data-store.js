...
function createDemoData(currentDate = new Date()) {
  ...
  return {
    ...
    sources,
    budgetLimits: {
      Housing: { "0000-00": round2(1350 + month * 10) },
      "Food & Groceries": { "0000-00": round2(520 + day) },
      Transport: { "0000-00": round2(230 + month * 4) },
      Entertainment: { "0000-00": round2(240 + (day % 5) * 15) },
      [`Travel ${year}`]: { "0000-00": 400 }
    },
    recurrences: [
      {
        id: crypto.randomUUID(),
...
function normalizeBudgetLimits(raw) {
  if (!raw || typeof raw !== "object") return {};
  const result = {};
  for (const [category, value] of Object.entries(raw)) {
    if (typeof value === "number" && value > 0) {
      // Old flat format — migrate to per-month using sentinel key
      result[category] = { "0000-00": Math.round((value + Number.EPSILON) * 100) / 100 };
    } else if (value && typeof value === "object" && !Array.isArray(value)) {
      const monthMap = {};
      for (const [mk, limit] of Object.entries(value)) {
        const limitNum = Number(limit);
        if (Number.isFinite(limitNum) && limitNum > 0) {
          monthMap[mk] = Math.round((limitNum + Number.EPSILON) * 100) / 100;
        }
      }
      if (Object.keys(monthMap).length > 0) result[category] = monthMap;
    }
  }
  return result;
}
...
function normalizeData(parsed) {
  return {
    ...
    budgetLimits: normalizeBudgetLimits(parsed.budgetLimits),
...
class DataStore {
  ...
  deleteCategory(name, currentMonthKey) {
    if (!this.data.categories.includes(name)) {
      throw new Error("Category not found");
    }
    const mk = currentMonthKey || "";
    const hasPastTransactions = this.data.transactions.some(
      (tx) => tx.category === name && (mk ? monthKeyFor(tx.date) < mk : true)
    );
    if (hasPastTransactions) {
      throw new Error(
        "Category cannot be deleted because it has transactions in previous months. " +
        "Historical data must be preserved."
      );
    }
    const hasCurrentTransactions = this.data.transactions.some((tx) => tx.category === name);
    if (hasCurrentTransactions) {
      throw new Error(
        "Category cannot be deleted because it still has transactions assigned to it. " +
        "Remove those transactions first."
      );
    }
  ...
  setBudgetLimit(category, rawLimit, monthKey) {
    if (!this.data.categories.includes(category)) {
      throw new Error("Category not found");
    }
    const mk = String(monthKey || "0000-00");
    const isBlank = rawLimit === "" || rawLimit === null || typeof rawLimit === "undefined";
    if (isBlank || Number(rawLimit) === 0) {
      if (this.data.budgetLimits[category]) {
        delete this.data.budgetLimits[category][mk];
        if (Object.keys(this.data.budgetLimits[category]).length === 0) {
    ...
    if (!this.data.budgetLimits[category]) {
      this.data.budgetLimits[category] = {};
    }
    this.data.budgetLimits[category][mk] = Math.round((limit + Number.EPSILON) * 100) / 100;
    this.saveCurrent();
  }
